    ...
    useEffect(() => {
        ...
        class Band {
            constructor(w, h, side) {
                ...
                // Color: soft blues, purples, teals (light-mode friendly)
                const palettes = [
                    { r: 59, g: 126, b: 248 },   // Brand blue
            ...
            draw(ctx, t) {
                ...
                for (let i = 0; i < this.numBars; i++) {
                    ...
                    const alpha = edgeFade * (0.05 + this.depth * 0.09) * (0.5 + flicker * 0.3);
        ...
        const render = () => {
            ...
            const bgColor     = isDark ? '#080b12' : '#f5f6fa';
            const vignetteRgb = isDark ? '8,11,18'  : '245,246,250';
            const vigStr      = isDark
                ? [0.88, 0.55, 0]
                : [0.88, 0.48, 0];  // lighter center in light mode
            ...
            grad.addColorStop(0,    `rgba(${vignetteRgb},${vigStr[0]})`);
            grad.addColorStop(0.45, `rgba(${vignetteRgb},${vigStr[1]})`);
            grad.addColorStop(1,    `rgba(${vignetteRgb},${vigStr[2]})`);
            ctx.fillStyle = grad;
            ctx.fillRect(0, 0, w, h);

            // Edge fade
            const edgeGrad = ctx.createLinearGradient(0, 0, 0, h);
            edgeGrad.addColorStop(0,    `rgba(${vignetteRgb},0.55)`);
            edgeGrad.addColorStop(0.15, `rgba(${vignetteRgb},0)`);
            edgeGrad.addColorStop(0.85, `rgba(${vignetteRgb},0)`);
            edgeGrad.addColorStop(1,    `rgba(${vignetteRgb},0.65)`);
